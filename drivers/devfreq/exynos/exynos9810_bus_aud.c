/* linux/drivers/devfreq/exynos/exynos9810_bus_aud.c
 *
 * Copyright (c) 2015 Samsung Electronics Co., Ltd.
 *              http://www.samsung.com
 *
 * Samsung EXYNOS9810 SoC AUD devfreq driver
 *
...
static int exynos9810_devfreq_aud_cmu_dump(struct exynos_devfreq_data *data)
{
...
static int exynos9810_devfreq_aud_reboot(struct exynos_devfreq_data *data)
{
/*
...
static int exynos9810_devfreq_aud_get_freq(struct device *dev, u32 *cur_freq,
...
static int exynos9810_devfreq_aud_set_freq(struct device *dev, u32 new_freq,
		struct clk *clk, struct exynos_devfreq_data *data)
{
...
static int exynos9810_devfreq_aud_resume(struct exynos_devfreq_data *data)
{
	u32 cur_freq = 0;

	/* for sync from resume frequency */
	if (exynos9810_devfreq_aud_get_freq(data->dev, &cur_freq, data->clk, data)) {
...
static int exynos9810_devfreq_aud_init_freq_table(struct exynos_devfreq_data *data)
{
...
static int __init exynos9810_devfreq_aud_init_prepare(struct exynos_devfreq_data *data)
{
	data->ops.get_freq = exynos9810_devfreq_aud_get_freq;
	data->ops.set_freq = exynos9810_devfreq_aud_set_freq;
	data->ops.init_freq_table = exynos9810_devfreq_aud_init_freq_table;
	data->ops.resume = exynos9810_devfreq_aud_resume;
	data->ops.reboot = exynos9810_devfreq_aud_reboot;
	data->ops.cmu_dump = exynos9810_devfreq_aud_cmu_dump;

	return 0;
}

static int __init exynos9810_devfreq_aud_initcall(void)
{
	if (register_exynos_devfreq_init_prepare(DEVFREQ_AUD,
				exynos9810_devfreq_aud_init_prepare))
		return -EINVAL;

	return 0;
}

fs_initcall(exynos9810_devfreq_aud_initcall);
